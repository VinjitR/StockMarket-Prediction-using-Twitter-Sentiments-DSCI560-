...
export default function AAPL() {
  ...
  useEffect(() => {
    ...
    fetch('https://www.alphavantage.co/query?function=OVERVIEW&symbol=AAPL&apikey=7NT5QY9WX6BZKLQ6')
    ...
    fetch("http://localhost:5000/stockdata/AAPL")
    .then(response => response.json())
  ...
  const fetchARIMA = async(targetsize) => {
    setLoadA(true);
    alert("Switching to ARIMA AAPL "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/ARIMA/"+targetsize.toString()+"/AAPL")
    .then(response => response.json())
    .then(data=>get_date(data))  
  ...
  const fetchLSTM = async(targetsize) => {
    alert("Switching to LSTM AAPL "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/LSTM/"+targetsize.toString()+"/AAPL")
    .then(response => response.json())
    .then(data=>get_date(data))
...
const fetchTwitterPie= async()=>{
  fetch('http://localhost:5000/tweetper/AAPL')
  .then(response => response.json())
