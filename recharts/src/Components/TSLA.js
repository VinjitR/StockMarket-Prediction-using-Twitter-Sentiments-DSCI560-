...
export default function TSLA() {
  ...
  useEffect(() => {
    ...
    fetch('https://www.alphavantage.co/query?function=OVERVIEW&symbol=TSLA&apikey=7NT5QY9WX6BZKLQ6')
    ...
    fetch("http://localhost:5000/stockdata/TSLA")
    .then(response => response.json())
  ...
  const fetchARIMA = async(targetsize) => {
    setLoadA(true);
    alert("Switching to ARIMA TSLA "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/ARIMA/"+targetsize.toString()+"/TSLA")
    .then(response => response.json())
    .then(data=>get_date(data))  
  ...
  const fetchLSTM = async(targetsize) => {
    alert("Switching to LSTM TSLA "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/LSTM/"+targetsize.toString()+"/TSLA")
    .then(response => response.json())
    .then(data=>get_date(data))
...
const fetchTwitterPie= async()=>{
  fetch('http://localhost:5000/tweetper/TSLA')
  .then(response => response.json())
