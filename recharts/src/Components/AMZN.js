...
export default function AMZN() {
  ...
  useEffect(() => {
    ...
    fetch('https://www.alphavantage.co/query?function=OVERVIEW&symbol=AMZN&apikey=7NT5QY9WX6BZKLQ6')
    ...
    fetch("http://localhost:5000/stockdata/AMZN")
    .then(response => response.json())
  ...
  const fetchARIMA = async(targetsize) => {
    setLoadA(true);
    alert("Switching to ARIMA AMZN "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/ARIMA/"+targetsize.toString()+"/AMZN")
    .then(response => response.json())
    .then(data=>get_date(data))  
  ...
  const fetchLSTM = async(targetsize) => {
    alert("Switching to LSTM AMZN "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/LSTM/"+targetsize.toString()+"/AMZN")
    .then(response => response.json())
    .then(data=>get_date(data))
...
const fetchTwitterPie= async()=>{
  fetch('http://localhost:5000/tweetper/AMZN')
  .then(response => response.json())
