...
export default function GOOGL() {
  ...
  useEffect(() => {
    ...
    fetch('https://www.alphavantage.co/query?function=OVERVIEW&symbol=GOOGL&apikey=7NT5QY9WX6BZKLQ6')
    ...
    fetch("http://localhost:5000/stockdata/GOOGL")
    .then(response => response.json())
  ...
  const fetchARIMA = async(targetsize) => {
    setLoadA(true);
    alert("Switching to ARIMA GOOGL "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/ARIMA/"+targetsize.toString()+"/GOOGL")
    .then(response => response.json())
    .then(data=>get_date(data))  
  ...
  const fetchLSTM = async(targetsize) => {
    alert("Switching to LSTM GOOGL "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/LSTM/"+targetsize.toString()+"/GOOGL")
    .then(response => response.json())
    .then(data=>get_date(data))
...
const fetchTwitterPie= async()=>{
  fetch('http://localhost:5000/tweetper/GOOGL')
  .then(response => response.json())
