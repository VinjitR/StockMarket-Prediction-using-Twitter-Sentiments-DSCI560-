...
export default function FB() {
  ...
  useEffect(() => {
    ...
    fetch('https://www.alphavantage.co/query?function=OVERVIEW&symbol=FB&apikey=7NT5QY9WX6BZKLQ6')
    ...
    fetch("http://localhost:5000/stockdata/FB")
    .then(response => response.json())
  ...
  const fetchARIMA = async(targetsize) => {
    setLoadA(true);
    alert("Switching to ARIMA FB "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/ARIMA/"+targetsize.toString()+"/FB")
    .then(response => response.json())
    .then(data=>get_date(data))  
  ...
  const fetchLSTM = async(targetsize) => {
    alert("Switching to LSTM FB "+targetsize.toString()+"day predictions")
    fetch("http://localhost:5000/LSTM/"+targetsize.toString()+"/FB")
    .then(response => response.json())
    .then(data=>get_date(data))
...
const fetchTwitterPie= async()=>{
  fetch('http://localhost:5000/tweetper/FB')
  .then(response => response.json())
